...
class ControllerExtensionPaymenthyperpayclicktopay extends Controller
{
    ...
    {
        $this->language->load('extension/payment/hyperpay_click_to_pay');
        $this->load->model('checkout/order');
        $data['button_confirm'] = $this->language->get('button_confirm');
        //--------------------------------------
        $testMode = $this->config->get('payment_hyperpay_click_to_pay_testmode');
        ...
        $channel = $this->config->get('payment_hyperpay_click_to_pay_channel');
        $mode = $this->config->get('payment_hyperpay_click_to_pay_trans_mode');
        ...
        $ip = $order_info['ip'];
        $tokenization = $customer_id > 0 ? $this->config->get('payment_hyperpay_click_to_pay_tokenization_status') : 0;
        ...
        $data['formStyle'] = $this->config->get('payment_hyperpay_click_to_pay_payment_style');
    ...
    public function callback()
    {
        if (isset($_GET['id'])) {
            ...
            $url .= "?entityId=" . trim($this->config->get('payment_hyperpay_click_to_pay_channel'));
            $accesstoken = $this->config->get('payment_hyperpay_click_to_pay_accesstoken');
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            ...
            if ($order_info) {
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_hyperpay_click_to_pay_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                ...
                if ($this->config->get('payment_hyperpay_click_to_pay_mailerrors') == 1) {
    ...
    public function fail()
    {
        ...
        $data['heading_title'] = $this->config->get('payment_hyperpay_click_to_pay_heading_title');

        if (isset($this->session->data['payment_hyperpay_click_to_pay_error'])) {
            $data['general_error'] = $this->session->data['payment_hyperpay_click_to_pay_error'];
        } else {
