...
class ControllerExtensionPaymentHyperpaySamsungpay extends Controller
{
    ...
    {
        $this->language->load('extension/payment/hyperpay_samsungpay');
        $this->load->model('checkout/order');
        $data['button_confirm'] = $this->language->get('button_confirm');
        //--------------------------------------
        $testMode = $this->config->get('payment_hyperpay_samsungpay_testmode');
        if ($testMode == 0) {
        ...
        $channel = $this->config->get('payment_hyperpay_samsungpay_channel');
        $mode = $this->config->get('payment_hyperpay_samsungpay_trans_mode');
        $token = $this->config->get('payment_hyperpay_samsungpay_accesstoken');
        $type = $this->config->get('payment_hyperpay_samsungpay_trans_type');
        $connector = $this->config->get('payment_hyperpay_samsungpay_connector');
        $currency = $this->config->get('payment_hyperpay_samsungpay_base_currency');
        ...
        $payment_brands = implode(' ', $this->config->get('payment_hyperpay_samsungpay_brands'));
        //--------------------------------------
        $data['token'] = $token;
        $data['payment_brands'] = $payment_brands;
        $data['scriptURL'] = $scriptURL . $token;

        $data['formStyle'] = $this->config->get('payment_hyperpay_samsungpay_payment_style');
        ...
        $data['postbackURL'] = $url . 'index.php?route=extension/payment/hyperpay_samsungpay/callback';

        return $this->load->view('extension/payment/hyperpay_samsungpay', $data);
    }
    ...
    public function callback()
    {
        if (isset($_GET['id'])) {
            ...
            $url .= "?entityId=" . trim($this->config->get('payment_hyperpay_samsungpay_channel'));
            $accesstoken = $this->config->get('payment_hyperpay_samsungpay_accesstoken');
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            ...
            if ($order_info) {
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_hyperpay_samsungpay_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                    // Order is not approved.
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_hyperpay_samsungpay_order_status_failed_id'), '', TRUE);
                    $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg . Order Id: $orderid");
                    $this->session->data['payment_hyperpay_samsungpay_error'] = $failed_msg;
                    $this->response->redirect($this->url->link('extension/payment/hyperpay_samsungpay/fail', '', true));
                }
                exit;
            } else {
                if ($this->config->get('payment_hyperpay_samsungpay_mailerrors') == 1) {
                ...
                //$this->model_checkout_order->confirm($orderid, $this->config->get('payment_hyperpay_samsungpay_order_status_failed_id'), '', TRUE);
                $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_hyperpay_samsungpay_order_status_failed_id'), '', TRUE);
                $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg. Order Id: $orderid");
                $this->response->redirect($this->url->link('extension/payment/hyperpay_samsungpay/fail', '', true));
                exit;
    ...
    {
        $this->language->load('extension/payment/hyperpay_samsungpay');
        $data['heading_title'] = $this->config->get('payment_hyperpay_samsungpay_heading_title');

        if (isset($this->session->data['payment_hyperpay_samsungpay_error'])) {
            $data['general_error'] = $this->session->data['payment_hyperpay_samsungpay_error'];
        } else {
