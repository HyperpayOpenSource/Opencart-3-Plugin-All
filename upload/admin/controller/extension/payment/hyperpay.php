...
class ControllerExtensionPaymentHyperpay extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/hyperpay', 'user_token=' . $this->session->data['user_token'], true),
            'separator' => ' :: '
        );
        ...
        $data['action'] = $this->url->link('extension/payment/hyperpay', 'user_token=' . $this->session->data['user_token'], true);
        ...
        if (isset($this->request->post['payment_hyperpay_status'])) {
            $data['payment_hyperpay_status'] = $this->request->post['payment_hyperpay_status'];
        } else {
            $data['payment_hyperpay_status'] = $this->config->get('payment_hyperpay_status');
        }

        if (isset($this->request->post['payment_hyperpay_base_currency'])) {
            $data['payment_hyperpay_base_currency'] = $this->request->post['payment_hyperpay_base_currency'];
        } else {
            $data['payment_hyperpay_base_currency'] = $this->config->get('payment_hyperpay_base_currency');
        }

        if (isset($this->request->post['payment_hyperpay_sort_order'])) {
            $data['payment_hyperpay_sort_order'] = $this->request->post['payment_hyperpay_sort_order'];
        } elseif($this->config->get('payment_hyperpay_sort_order')) {
            $data['payment_hyperpay_sort_order'] = $this->config->get('payment_hyperpay_sort_order');
        }else{
            $data['payment_hyperpay_sort_order'] = 2;
        }

        if (isset($this->request->post['payment_hyperpay_testmode'])) {
            $data['payment_hyperpay_testmode'] = $this->request->post['payment_hyperpay_testmode'];
        } else {
            $data['payment_hyperpay_testmode'] = $this->config->get('payment_hyperpay_testmode');
        }

        if (isset($this->request->post['payment_hyperpay_trans_type'])) {
            $data['payment_hyperpay_trans_type'] = $this->request->post['payment_hyperpay_trans_type'];
        } else {
            $data['payment_hyperpay_trans_type'] = $this->config->get('payment_hyperpay_trans_type');
        }

        if (isset($this->request->post['payment_hyperpay_trans_mode'])) {
            $data['payment_hyperpay_trans_mode'] = $this->request->post['payment_hyperpay_trans_mode'];
        } else {
            $data['payment_hyperpay_trans_mode'] = $this->config->get('payment_hyperpay_trans_mode');
        }

        if (isset($this->request->post['payment_hyperpay_heading_title'])) {
            $data['payment_hyperpay_heading_title'] = $this->request->post['payment_hyperpay_heading_title'];
        } else {
            $data['payment_hyperpay_heading_title'] = $this->config->get('payment_hyperpay_heading_title');
        }

        if (isset($this->request->post['payment_hyperpay_channel'])) {
            $data['payment_hyperpay_channel'] = $this->request->post['payment_hyperpay_channel'];
        } else {
            $data['payment_hyperpay_channel'] = $this->config->get('payment_hyperpay_channel');
        }

        if (isset($this->request->post['payment_hyperpay_accesstoken'])) {
            $data['payment_hyperpay_accesstoken'] = $this->request->post['payment_hyperpay_accesstoken'];
        } else {
            $data['payment_hyperpay_accesstoken'] = $this->config->get('payment_hyperpay_accesstoken');
        }
        $data['payment_hyperpay_connector'] = $this->config->get('payment_hyperpay_connector');
        if (isset($this->request->post['payment_hyperpay_connector'])) {
            $data['payment_hyperpay_connector'] = $this->request->post['payment_hyperpay_connector'];
        }
        if (isset($this->request->post['payment_hyperpay_brands'])) {
            $data['payment_hyperpay_brands'] = $this->request->post['payment_hyperpay_brands'];
        } else {
            $data['payment_hyperpay_brands'] = $this->config->get('payment_hyperpay_brands');
        }

        if (isset($this->request->post['payment_hyperpay_payment_style'])) {
            $data['payment_hyperpay_payment_style'] = $this->request->post['payment_hyperpay_payment_style'];
        } else {
            $data['payment_hyperpay_payment_style'] = $this->config->get('payment_hyperpay_payment_style');
        }

        if (isset($this->request->post['payment_hyperpay_mailerrors'])) {
            $data['payment_hyperpay_mailerrors'] = $this->request->post['payment_hyperpay_mailerrors'];
        } else {
            $data['payment_hyperpay_mailerrors'] = $this->config->get('payment_hyperpay_mailerrors');
        }

        if (isset($this->request->post['payment_hyperpay_mailerrors_enable'])) {
            $data['payment_hyperpay_mailerrors_enable'] = $this->request->post['payment_hyperpay_mailerrors_enable'];
        } else {
            $data['payment_hyperpay_mailerrors_enable'] = $this->config->get('payment_hyperpay_mailerrors_enable');
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/hyperpay', $data));
    }

    private function validate()
    {
        if (!$this->user->hasPermission('modify', 'extension/payment/hyperpay')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }


        if (!$this->request->post['payment_hyperpay_heading_title']) {
            $this->error['heading_title'] = $this->language->get('error_heading_title');
        }

        if (!$this->request->post['payment_hyperpay_channel']) {
            $this->error['channel'] = $this->language->get('error_channel');
        }

        if (!$this->request->post['payment_hyperpay_accesstoken']) {
            $this->error['access'] = $this->language->get('error_access');
        }

        if (!$this->request->post['payment_hyperpay_connector']) {
